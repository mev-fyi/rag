...
@timeit
def create_index(add_new_transcripts=False, num_files=2):
    ...
    pipeline_v1 = initialise_pipeline(add_to_vector_store=False)
    nodes = pipeline_v1.run(documents=documents_pdfs + documents_youtube, num_workers=int(os.cpu_count())-3, show_progress=True)
    ...
    pipeline_v2 = initialise_pipeline(delete_old_index=True)
