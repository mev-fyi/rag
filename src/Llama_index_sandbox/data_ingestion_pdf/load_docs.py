...
def load_single_pdf(file_path, title_extraction_func: Callable, author: str, release_date: str, loader=PyMuPDFReader(),
                    debug=False):
    try:
...
@timeit
def load_pdfs(directory_path: Union[str, Path], title_extraction_func: Callable, author: str, release_date: str, num_files: int = None, debug=False):
    if not isinstance(directory_path, Path):
        directory_path = Path(directory_path)

    all_documents = []
    partial_load_single_pdf = partial(load_single_pdf, title_extraction_func=title_extraction_func, author=author, release_date=release_date, debug=debug)
