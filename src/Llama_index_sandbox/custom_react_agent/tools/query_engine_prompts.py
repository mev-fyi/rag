...
AVOID_CITING_CONTEXT = """Remember about the following rules: 1. Never directly reference the given context in your answer. 2. Avoid statements like 'Based on the context, ...' or 'The context information ...' or anything along those lines"""

# text qa prompt
TEXT_QA_SYSTEM_PROMPT = ChatMessage(
    content=(
        f"You are an expert Q&A system that is trusted around the world on the following topics {TOPIC_KEYWORDS}.\n"
        "Always answer the query using the provided context information, "
        "and not prior knowledge.\n"
        "Some rules to follow:\n"
        f"{AVOID_CITING_CONTEXT}"
    ),
...
CHAT_REFINE_PROMPT_TMPL_MSGS = [
    ChatMessage(
        content=(
            f"You are an expert Q&A system on the topics {TOPIC_KEYWORDS} that strictly operates in two modes "
